...
function resetDashboard() {
  ...
  if (response == ui.Button.YES) {
    ...
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(DASHBOARD_NAME);
    if (sheet) {
...
function startCopyJob(jobConfig) {
  ...
  let newMainFolder;
  if (isDryRun) {
    // Dalam dry run, kita tidak membuat folder sungguhan.
...
function executeCopyJob() {
  ...
  try {
    ...
    throw new Error(`Proses berhenti karena error. Lihat Mission Control.`);
  }
}
...
function copyFolderRecursive(source, target, currentPath, isDryRun, skipExisting) {
  ...
  while(files.hasNext()){
    ...
    if (skipExisting && existingItems.has(fileName)) {
        itemsToLog.push(['⏩ Dilewati', fileName, 'File', currentPath, file.getUrl(), 'Sudah ada']);
        incrementStats('skipped');
    } else {
        if (isDryRun) {
            itemsToLog.push(['✔️ Akan Disalin', fileName, 'File', currentPath, file.getUrl()]);
            incrementStats('success');
        } else {
            updateStatus(`Menyalin file: ${currentPath}${fileName}`);
            try {
                const copiedFile = file.makeCopy(fileName, target);
                itemsToLog.push(['✅ Sukses', fileName, 'File', currentPath, file.getUrl(), copiedFile.getUrl()]);
                incrementStats('success');
            } catch(e) {
                itemsToLog.push(['❌ Gagal', fileName, 'File', currentPath, file.getUrl(), '', e.message]);
  ...
  if (itemsToLog.length > 0) {
      logToDashboard(itemsToLog);
      incrementProcessedCount(itemsToLog.length);
      itemsToLog.length = 0; // Clear the array
  }
  ...
  while(subFolders.hasNext()){
      ...
      if (skipExisting && existingItems.has(folderName)) {
          logToDashboard([['⏩ Dilewati', folderName, 'Folder', currentPath, subFolder.getUrl(), 'Sudah ada']]);
          const subItemsCount = countItemsRecursive(subFolder).total;
          incrementProcessedCount(subItemsCount + 1); // Skip folder + its content
          incrementStats('skipped', subItemsCount + 1);
          if (!isDryRun) {
...
function clearPreviousJob() {
  a_props.deleteAllProperties();
  a_cache.remove('existingItemsCache');
}
...
function getExistingItems(targetFolder, isDryRun) {
  ...
  const folders = targetFolder.getFolders(); while (folders.hasNext()) { const f = folders.next(); items.set(f.getName(), f); }
...
function logToDashboard(dataRows) {
  ...
  const now = new Date();
  dataRows.forEach(row => row.push(now));
  
  sheet.getRange(lastRow, 1, numRows, numCols + 1).setValues(dataRows);
}
...
function incrementProcessedCount(count = 1) {
  ...
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(DASHBOARD_NAME);
  if (sheet) sheet.getRange('C4').setValue(current);
}
...
function incrementStats(type, count = 1) {
  ...
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(DASHBOARD_NAME);
  if (sheet) {
      const cellMap = { success: 'F5', skipped: 'F6', failed: 'F7' };
      if (cellMap[type]) {
...
function setupDashboard(sourceFolder, targetFolder, totalItems, isDryRun) {
    ...
    sheet.getRange('A1:H8').setFontFamily('Google Sans').setVerticalAlignment('middle');
    ...
    // Info
    sheet.getRange('A3').setValue(`Sumber: ${sourceFolder.getName()}`);
    sheet.getRange('A4').setValue(`Tujuan: ${isDryRun ? '(Simulasi)' : targetFolder.getName()}`);

    // Progress Panel
    sheet.getRange('A6').setValue('Status:');
    sheet.getRange('C6').setValue('Progres Total:');
    sheet.getRange('B6').setFontSize(14).setFontWeight('bold');

    sheet.getRange('D6').setFormula('=SPARKLINE(E6, {"charttype","bar";"max",F6;"color1","#4285f4"})').setNumRows(2);
    sheet.getRange('E6').setValue(0); // Processed
    sheet.getRange('F6').setValue(totalItems); // Total
    sheet.getRange('G6').setFormula('=IFERROR(E6/F6, 0)').setNumberFormat('0.00%');
    ...
    // Log Headers
    const headers = ['Status', 'Nama Item', 'Tipe', 'Lokasi di Folder Baru', 'Link Asli', 'Link Hasil Copy', 'Pesan', 'Timestamp'];
    sheet.getRange(9, 1, 1, headers.length).setValues([headers]).setBackground('#f3f3f3').setFontWeight('bold');
    
    // Conditional Formatting
    const dataRange = sheet.getRange('A10:H');
    sheet.clearConditionalFormatRules();
    const rules = [
      SpreadsheetApp.newConditionalFormatRule().whenTextContains('❌').setBackground('#fbe5d6').setRanges([dataRange]).build(),
      SpreadsheetApp.newConditionalFormatRule().whenTextContains('⏩').setFontColor('#999999').setRanges([dataRange]).build(),
      SpreadsheetApp.newConditionalFormatRule().whenTextContains('✔️').setFontColor('#0b804b').setRanges([dataRange]).build()
    ];
    sheet.setConditionalFormatRules(rules);

    // Column Widths
    sheet.setColumnWidths(1, 1, 120); // Status
    sheet.setColumnWidths(2, 3, 250); // Nama, Tipe, Lokasi
...
function updateDashboardStatus(text, color) {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(DASHBOARD_NAME);
  if (sheet) {
    sheet.getRange('B6').setValue(text).setFontColor(color);
