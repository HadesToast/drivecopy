 ...
 * dan fitur profesional seperti Mode Validasi (Dry Run) dan Buat Arsip ZIP.
 * 
 * -- REVISI FINAL v3 (Lengkap) --
 * - FITUR: Opsi untuk membuat arsip .ZIP setelah penyalinan selesai.
 * - PERBAIKAN: Mengatasi error 'Jumlah baris tidak cocok' & 'Error mengurai formula'.
 * - KUALITAS: Kode bersih, penanganan error yang kuat, dan alur yang jelas.
 */
...
function startCopyJob(jobConfig) {
  ...
  try {
    const { sourceFolderUrl, newFolderName, destinationFolderUrl, skipExisting, isDryRun, createZip } = jobConfig;
...
function executeCopyJob() {
  try {
    ...
    const zipRequested = state.createZip === 'true';
    ...
    if (a_props.getProperty('stopRequested') !== 'true') {
        ...
        const newFolderId = isDryRun ? null : targetFolder.getId();
        
        // Jangan hapus properti jika akan membuat ZIP, hapus nanti setelah proses ZIP selesai
        if (!zipRequested || isDryRun) {
            clearPreviousJob();
        }
...
// =================================================================
// SECTION: FUNGSI UNTUK MEMBUAT ZIP
// =================================================================
function createZipFromFolder(folderId) {
  try {
    const folder = DriveApp.getFolderById(folderId);
    const folderName = folder.getName();
    const parentFolder = folder.getParents().next();

    Logger.log(`Memulai proses ZIP untuk folder: ${folderName}`);
    updateDashboardStatus(`🗜️ MENGARSIPKAN KE ZIP...`, '#673ab7');

    const blobs = [];
    collectBlobsRecursive(folder, '', blobs);

    if (blobs.length === 0) {
      throw new Error("Tidak ada file yang ditemukan di dalam folder untuk di-zip.");
    }

    Logger.log(`Mengumpulkan ${blobs.length} file. Membuat arsip ZIP...`);
    const zipBlob = Utilities.zip(blobs, `${folderName}.zip`);

    Logger.log(`Arsip ZIP dibuat. Menyimpan ke Drive...`);
    const zipFile = parentFolder.createFile(zipBlob);
    
    updateDashboardStatus(`✅ ARSIP ZIP SELESAI`, '#0f9d58');
    logToDashboard([['✅ Sukses', `${folderName}.zip`, 'Arsip ZIP', '/', '', zipFile.getUrl()]]);
    
    clearPreviousJob();

    return { status: 'success', zipUrl: zipFile.getUrl() };

  } catch (e) {
    Logger.log(`Gagal membuat ZIP: ${e.toString()}`);
    updateDashboardStatus(`⛔ GAGAL MEMBUAT ZIP`, '#db4437');
    logToDashboard([['❌ Gagal', 'Proses ZIP', 'Sistem', '/', '', '', `Error: ${e.message}`]]);
    clearPreviousJob();
...
/**
 * Helper function untuk mengumpulkan semua file blob secara rekursif
 */
function collectBlobsRecursive(folder, path, blobs) {
  ...
  while (files.hasNext()) {
    ...
    try {
      ...
      Logger.log(`Gagal membaca file: ${file.getName()} untuk di-zip. Error: ${e.message}. File akan dilewati.`);
      logToDashboard([['⚠️ Dilewati (ZIP)', file.getName(), 'File', path, file.getUrl(), '', `Tidak bisa dibaca: ${e.message}`]]);
    }
  }
  ...
  while (subFolders.hasNext()) {
    const subFolder = subFolders.next();
    collectBlobsRecursive(subFolder, path + subFolder.getName() + '/', blobs);
  }
}
...
function setupDashboard(sourceFolder, targetFolder, totalItems, isDryRun) {
    ...
    const rules = [
      ...
      SpreadsheetApp.newConditionalFormatRule().whenTextStartsWith('⚠️').setBackground('#fff0e0').setRanges([dataRange]).build()
    ];
